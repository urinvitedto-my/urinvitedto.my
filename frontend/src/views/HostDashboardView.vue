...
/** Host names excluding "Admin" for use in invite messages. */
const visibleHosts = computed(() =>
  eventHosts.value.filter((h) => h.displayName !== 'Admin'),
)
 ...
 * "our wedding" for multi-host types, "{host}'s party" / "{host}'s birthday" otherwise.
 */
...
/** Generates the invite message text with tone based on event type. */
...
function buildMessageBody(eventType: string, label: string, eventLabel: string): string[] {
  ...
  // party (and any future casual types)
  return [
    `Hey ${label}! 🎉`,
    '',
