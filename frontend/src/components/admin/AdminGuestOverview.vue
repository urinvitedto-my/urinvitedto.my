...
import type { AdminEvent, AdminInvite } from "@/types"
...
const hostsRef = computed(() => props.event.hosts)
const { buildInviteMessage } = useInviteMessage(eventRef, hostsRef)

const invites = computed(() => adminStore.getInvites(props.event.id))
const loading = computed(() => adminStore.isSubLoading("invites", props.event.id))
const error = computed(() => adminStore.getSubError("invites", props.event.id))
...
function openInviteModal(guest: FlatGuest) {
  const invite = invites.value.find((i) => i.id === guest.inviteId)
  if (!invite) return
...
onMounted(() => adminStore.fetchInvites(props.event.id))
</script>
...
<template>
  <div>
    ...
    <template v-else>
        ...
        >
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2 flex-wrap">
              <button
                @click="openInviteModal(guest)"
                class="text-sm font-semibold text-primary hover:underline cursor-pointer text-left"
              >
                {{ guest.displayName }}
              </button>
              <span
      ...
      <Teleport to="body">
        <div
          v-if="showInviteModal && selectedInvite"
        ...
        >
          ...
          >
            ...
            >
              <h3 class="text-lg font-semibold text-primary">Invite Message</h3>
              <button
            ...
            <div class="flex-1 overflow-y-auto p-6 pt-4">
              <div class="border border-muted rounded-lg p-4">
                <div class="flex items-start justify-between gap-3 mb-2">
                  <div class="min-w-0">
                    <div class="font-medium text-primary truncate">
                      {{
                        ...
                        `Invite ${selectedInvite.inviteCode}`
                      }}
                    </div>
                    <div class="text-xs text-gray-500 mt-0.5">
                      Code:
                      <span class="font-mono font-medium">{{
                        selectedInvite.inviteCode
                      }}</span>
                      <span v-if="selectedInvite.guests.length" class="ml-2">
                        &middot;
                        {{
                          selectedInvite.guests
                            .map((g) => g.displayName)
                            .join(", ")
                        }}
                      </span>
                  ...
                  >
                    <svg
                      v-if="copiedInviteId === selectedInvite.id"
                      class="w-3.5 h-3.5"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2.5"
