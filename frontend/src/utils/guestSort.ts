...
export type GuestSortMode = 'name-asc' | 'name-desc' | 'responded-newest' | 'responded-oldest'
...
export function compareGuests(a: GuestSortableRow, b: GuestSortableRow, mode: GuestSortMode): number {
  switch (mode) {
    case 'name-asc':
 ...
 */
function rsvpSortKey(rsvpAt: string | null | undefined, order: 'newest' | 'oldest'): number {
  if (!rsvpAt) {
